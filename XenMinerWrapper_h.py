...
class MinerApp(tk.Tk):
    ...
    def __init__(self):
        ...
        self.title("XEN.pub's XenMiner Wrapper @modded by hannescrypto")
        ...
        self.miner_location = tk.Entry(self, width=100)
        ...
        self.eth_address = tk.Entry(self, width=100)
        ...
        self.python_env = tk.Entry(self, width=100)
        ...
        max_parallel = psutil.cpu_count(logical=True)  # get the number of physical cores
        max_parallel2 = psutil.cpu_count(logical=False)
        self.num_parallel = ttk.Combobox(self, values=[str(i) for i in range(1, max_parallel+1)])
        self.num_parallel.set("1")
        self.num_parallel.grid(row=3, column=1, padx=0, pady=10, sticky="w")
     ...
     #   self.parse_output_chk = tk.Checkbutton(self, text="Update", variable=self.parse_output_var)
      #  self.parse_output_chk.grid(row=3, column=1, padx=170, pady=2, sticky="w")
        ...
        tk.Label(self, text="CPU C/T").grid(row=3, column=1, padx=180, pady=2, sticky="w")
        tk.Label(self, text=max_parallel2).grid(row=3, column=1, padx=250, pady=10, sticky="w")
        
        tk.Label(self, text="/").grid(row=3, column=1, padx=265, pady=2, sticky="w")
        tk.Label(self, text=max_parallel).grid(row=3, column=1, padx=280, pady=10, sticky="w")
